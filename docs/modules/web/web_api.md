...
> **注意**：当前版本无认证机制，所有端点均可直接访问。用户角色系统将在后续版本实现。

| 端点 | 方法 | 说明 |
|------|------|------|
| `/health` | GET | 健康检查，返回系统状态 |
| `/api/cameras` | GET | 摄像头列表及运行状态 |
| `/api/alerts` | GET | 告警列表（分页、筛选） |
| `/api/alerts/{alert_id}` | GET | 单条告警详情 |
| `/api/alerts/{alert_id}/snapshot` | GET | 告警截图（返回图片二进制） |
| `/api/alerts/{alert_id}/clip` | GET | 告警视频片段（返回视频二进制） |
| `/api/alerts/{alert_id}/status` | PUT | 更新告警状态 |
| `/api/stats` | GET | 系统统计数据 |
| `/api/config` | GET | 系统配置（脱敏） |
| `/ws` | WebSocket | 实时推送连接（无需认证） |
...
当前版本无认证机制，所有端点均可直接访问。用户角色系统将在后续版本实现，计划包括：
- 用户名/密码登录
- JWT Token 认证
