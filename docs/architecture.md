# Vision Agent — 系统架构设计文档

## 一、项目定位

Vision Agent 是一个多路视频智能分析框架，核心价值主张：
...
```bash
# 最简单的外部监控脚本
*/1 * * * * curl -sf http://localhost:8080/health || echo "Vision Agent unhealthy" | mail -s "Alert" admin@example.com
```
...
- [ ] `python -m vision_agent --config configs/settings.yaml` 启动无报错
- [ ] `/health` 端点返回 200
- [ ] 各路摄像头状态为 "connected"
- [ ] Web 界面可正常访问
- [ ] 系统日志无异常，无敏感信息泄露
...
当前 Vision Agent 采用纯服务器架构（第一版）。后期根据规模扩展需求，可演进为端-云协同架构：
