...
1. **Agent 不 import Vision Agent 内部模块** — 所有专业工具通过 REST API 调用，Agent 只知道 URL 和 Token，不知道内部实现
2. **Vision Agent 只是第一个适配的后端** — 换一个视频监控系统，只需换一套 tools 实现，Agent 核心不动
3. **权限透传，不在 Agent 侧校验** — Agent 透传用户的 Bearer Token，Vision Agent 自己判断 403

## 耦合对比

```
之前（紧耦合）：                      现在（低耦合）：
agent/tools/camera.py                agent/adapters/vision_agent/camera.py
  import vision_agent.core.camera      POST http://localhost:8080/api/cameras
  pipeline.get_camera_states()         Authorization: Bearer {token}
  ↑ 代码级依赖                         ↑ 协议级依赖
```

专业工具从 4 组缩减到 3 组（规则管理待 Vision Agent 新增端点），MCP 通用工具独立。
...
```python
# agent/adapters/vision_agent/camera_tools.py
...
```
功能：单路摄像头详细信息
HTTP：GET /api/cameras/{camera_id}（需要 Vision Agent 新增此端点）
权限：viewer+
```
...
规则管理目前只有内部 YAML 文件操作，没有 REST API。需要先在 Vision Agent 侧新增：
...
与 Vision Agent 完全无关，Agent 自己的通用能力：
...
```python
# agent/tools.py（入口）
from agent.adapters.vision_agent.camera_tools import camera_tools
from agent.adapters.vision_agent.alert_tools import alert_tools
from agent.adapters.vision_agent.system_tools import system_tools
from agent.adapters.mcp.general_tools import load_mcp_general_tools

# 专业模式 — REST API → Vision Agent
professional_tools = camera_tools + alert_tools + system_tools
# + rule_tools 待 Vision Agent 新增 /api/rules 端点
...
## 权限矩阵（在 Vision Agent 侧校验）

Agent 不自己做权限判断。透传 token，403 → 展示错误给用户。

| Vision Agent API | viewer | operator | admin |
|---|---|---|---|
| GET /api/cameras | ✅ | ✅ | ✅ |
...
当 Agent 成熟后独立成仓库，零 Python 依赖 Vision Agent：
...
│   ├── memory/             # 四层记忆
│   ├── adapters/
│   │   ├── vision_agent/   # REST API 适配器 → 任何一个 Vision Agent 实例
│   │   └── mcp/            # MCP 通用工具
│   ├── skills/             # Skill 编排
│   ├── scheduler.py
│   └── web.py
└── configs/
    ├── agent.yaml           # adapters.vision_agent.base_url = "http://..."
    └── mcp_servers.yaml
```

Agent 只认 HTTP 协议和 MCP 协议，不认识任何 Vision Agent 内部代码。
